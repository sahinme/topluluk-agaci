...
function* getHomePostsSaga({ payload }) {
  console.log('sagaya girdi')
  try {
    if (payload.loaderStart) {
      yield put({ type: LOADER_START });

    }
    const response = yield call(Get, `post/paged-home-posts?PageNumber=${payload.pageNumber}&PageSize=${payload.pageSize}`, {}, {}, false);
...
function* getUnauthorizedPostsSaga({ payload }) {
  console.log('getUnauthorizedPostsSaga sagaya girdi')
  try {
